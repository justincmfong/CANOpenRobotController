...
const float controlLoopPeriodInms = 10.;   //!< Define the control loop period (in ms): the period of rt_control_thread loop (and so the app update rate). In ms.
const float CANUpdateLoopPeriodInms = 5; //!< Define the CAN PDO processing period. SYNCH messages (and so actual PDO update) is set to twice this period (twice slower). In ms.
